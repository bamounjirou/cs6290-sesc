...
if(!(tagsVisitedList.find(current_tag) !=tagsVisitedList.end()))
        ...
        tagsVisitedList.insert(current_tag); //Mycode now we will have the tag visited at least 1 going forward.
            ...
            {   //if(l && l->isValid() && l->oldTag ==current_tag)
            //if(l && (l->oldTag !=current_tag))
                {readCoheMiss.inc(); // tag was in a blokc but was invalidate
                }
              //  else {readReplMiss.inc(); // Everything else is a replacement miss.
                //        }
            }
else
{
...
void SMPCache::doWrite(MemRequest *mreq)
{
    ...
    if(!(tagsVisitedList.find(current_tag) !=tagsVisitedList.end()))
        ...
        tagsVisitedList.insert(current_tag); //Mycode now we will have the tag visited at least 1 going forward.
            ...
            {   //if(l && l->isValid() && l->oldTag ==current_tag)
            //if(l && (l->oldTag !=current_tag))
                {writeCoheMiss.inc(); // tag was in a block but was invalidate, that's a cohesive miss
                }
                //else {writeReplMiss.inc(); // Everything else is a replacement miss.
                 //   }
...
void SMPCache::realInvalidate(PAddr addr, ushort size, bool writeBack)
{
    while(size) {
		...
		l->oldTag = (uint32_t)current_tag; // Keep track of the tag being invalidated and later verify if same tag accessed again
		 //adding invalidated tag to database before actual invalidation	
                                       ...
                                       bool canDestroyCB)
{
    ...
    if(!l->isValid()) {
        ...
        DEBUGPRINT("   [%s] allocated free line for %x at %lld \n",
                   getSymbolicName(), addr , globalClock);
        //Mycode: Remove the tag old from the invalid stack as it's been accounted for already        
        tagsInvalidated.erase(l->oldTag);
