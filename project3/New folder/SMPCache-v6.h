...
protected:
    ...
    std::unordered_set<PAddr>tagsVisitedList; //Mycode. Data structure to track tag of addresses that's been visitied at least 1.
    std::unordered_set<PAddr>tagsInvalidated;// This list contains lines that supposed to be in FA  cache. If any tag is missed but is found here, that would be a capacity miss
    ...
    public:
        ...
        Entry() {
            outsResps = 0;
            cb = 0;
            invalidate = true; //Mycode changed from false to true
            std::cout<<"invalidate = true. \n";
            writeback = false;
        }
    };
    ...
    // BEGIN statistics
    //Mycode add co readCompMiss, readReplMiss, and readCoheMiss counters 
    GStatsCntr readCompMiss, readReplMiss, readCoheMiss,writeCompMiss, writeReplMiss, writeCoheMiss;
    const char* mylineSize;
