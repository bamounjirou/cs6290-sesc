...
void SMPCache::doRead(MemRequest *mreq)
{
    ...
    readMiss.inc();
//Mycode: start classifying 
PAddr current_tag = calcTag(addr);
if(!(tagsVisitedList.find(current_tag) !=tagsVisitedList.end()))
        {
        readCompMiss.inc();
        tagsVisitedList.insert(current_tag); //Mycode now we will have the tag visited at least 1 going forward.
        }
else if ( tagsInvalidated.find(current_tag) != tagsInvalidated.end())
            { 
                readCoheMiss.inc(); // tag was in a blokc but was invalidate, that's a capacity miss
            }
else
{
    readReplMiss.inc(); // Everything else is a replacement miss.
}
...
void SMPCache::doWrite(MemRequest *mreq)
{
    ...
    if(!(tagsVisitedList.find(current_tag) !=tagsVisitedList.end()))
        ...
        tagsVisitedList.insert(current_tag); //Mycode now we will have the tag visited at least 1 going forward.
                ...
                { 
                    writeCoheMiss.inc(); // tag was in a blokc but was invalidate, that's a capacity miss
                }
    else
    {
        writeReplMiss.inc(); // Everything else is a replacement miss.
    }
    //endcode
...
void SMPCache::realInvalidate(PAddr addr, ushort size, bool writeBack)
{
    while(size) {
		...
		PAddr current_tag= l->getTag();
		
        tagsInvalidated.insert(current_tag);  	
            l->invalidate();
        }
...
void SMPCache::concludeAccess(MemRequest *mreq)
{
    ...
    if(pendingInv.find(taddr)!=pendingInv.end()) {
        ...
        //Mycode: Add some code here to handle conlude mem access
        //PAddr current_tag= l->getTag();
        if (tagsInvalidated.find(taddr) != tagsInvalidated.end()){tagsInvalidated.erase(taddr);}
                                       ...
                                       bool canDestroyCB)
{
    ...
    if(!l->isValid()) {
        ...
        DEBUGPRINT("   [%s] allocated free line for %x at %lld \n",
                   getSymbolicName(), addr , globalClock);
        //Mycode: Remove the tag from the invalid stack as it's been accounted for already
        PAddr current_tag= l->getTag();
        if (tagsInvalidated.find(current_tag) != tagsInvalidated.end()){ tagsInvalidated.erase(current_tag);}
...
void SMPCache::doAllocateLine(PAddr addr, PAddr rpl_addr, CallbackBase *cb)
{
    ...
    if(!rpl_addr) {
        ...
        if(l) {
            ...
            l->changeStateTo(SMP_TRANS_RSV);
            cb->call();
            //Mycode: remove the tag from the list of invalids as a new line being allocated
            PAddr current_tag= l->getTag();
            if (tagsInvalidated.find(current_tag) != tagsInvalidated.end()){ tagsInvalidated.erase(current_tag);}
        }
    ...
    l->changeStateTo(SMP_TRANS_RSV);
    cb->call();
    //Mycode: remove the tag from the list of invalids as a new line being allocated via a new callback since first allocation may have failed
            PAddr current_tag= l->getTag();
            if (tagsInvalidated.find(current_tag) != tagsInvalidated.end()){ tagsInvalidated.erase(current_tag);}
}
