    ...
    , allocDirty("%s:allocDirty", name)
{
  ...
  tmpsection = section; // copying the section variable
  pStr = SescConf->getCharPtr(section, "replPolicy");

    MemObj *lowerLevel = NULL;
    //printf("%d\n", dms->getPID());  

// changed
invdb=new std::set<uint32_t>();  //initialising the database holding invalidated tags
    I(dms);
...
void SMPCache::doRead(MemRequest *mreq)
{
   PAddr addr = mreq->getPAddr();
    ...
    PAddr tag = calcTag(addr);	//calculating tag of block
    ...
    if (l && l->canBeRead()) {
		...
		if (capdb.size() == (SescConf->getInt(tmpsection, "size")/cache->getLineSize())) { 
...
//change made - prj3
		tag = l->getTag();
		l->prevTag = (uint32_t)tag;
		invdb->insert(tag); //adding invalidated tag to database before actual invalidation		
            l->invalidate();
        }
...
//invdb.erase(std::find(invdb.begin(),invdb.end(),l->prevTag));
invdb->erase(l->prevTag);
